...
// 保存点击图里所有选择器对应元素，方便删除重新渲染
  ...
  setToolbar: function(requrest_id,type,url){
    ...
    filterFlyout += "筛选展示元素";
    filterFlyout += '</div>'; // End of .header
    ...
    filterFlyout += '</div>'; // End of .footer
    filterFlyout += '</form></div>'; // End of flyout

    // 热力图筛选浮层，点击筛选图标显示/隐藏浮层
    ...
    // 热力图筛选浮层，点击确定按钮，先执行输入格式检查，再筛选元素
    $(document.body).on('click', '#sa_sdk_heatmap_filterFlyout .sa-sdk-heatmap-filterflyout-submitbtn', function() {
      ...
      if (type === 'between') {
        if (!isInRange(startInput.val())) {
          me.showErrorInfo(2, { error: errMsg1});
          return ;
        }
        ...
        if (Number(startInput.val()) > Number(endInput.val())) {
          me.showErrorInfo(2, { error: errMsg2});
          return ;
        }
    ...
    function filterClickMap(type, val1, val2) {
      var inRange = function(x) {
        var n1 = Number(val1), n2 = Number(val2);
        if (type === 'gt') {
          return x > n1;
        } else if (type === 'lt') {
          return x < n1;
    ...
    _.each(data,function(obj,key){
      if(obj.by_values[0]){
        ...
        if(typeof selector === 'object' && selector.length > 0){
          ...
          // 使用定时器异步操作避免元素选择器选择元素不对的问题
          setTimeout(function() {me.renderHeatData(selector,obj,key);}, 100);
        }
      }
    });
...
var heatmap = {
  ...
  prepare: function(data,type,url){
    var me = this;
      ...
      if(sd.errorMsg){
          heatmap_render.showErrorInfo(2,{error: sd.errorMsg});
      }
